...
const RateChart = ({ selectedCurrency }) => {
        ...
        if (isChartHovered && historyData.length > 0) {
            ...
            historyData.forEach((item, index) => {
                ...
                const midDistance = Math.sqrt(
                    Math.pow(chartCoordX - pointX, 2) +
                        Math.pow(chartCoordY - midY, 2)
                );
                if (
                    midDistance < proximityThreshold &&
                    midDistance < minDistance
                ) {
                ...
                if (item.buy) {
                    ...
                    const buyDistance = Math.sqrt(
                        Math.pow(chartCoordX - pointX, 2) +
                            Math.pow(chartCoordY - buyY, 2)
                    );
                    if (
                        buyDistance < proximityThreshold &&
                        buyDistance < minDistance
                    ) {
                ...
                if (item.sell) {
                    ...
                    const sellDistance = Math.sqrt(
                        Math.pow(chartCoordX - pointX, 2) +
                            Math.pow(chartCoordY - sellY, 2)
                    );
                    if (
                        sellDistance < proximityThreshold &&
                        sellDistance < minDistance
                    ) {
    ...
    const getY = (value) =>
        range > 0
            ? marginTop +
              chartHeight -
              ((parseFloat(value) - minRate) / range) * chartHeight
            ...
            {hoveredPoint && chartRef && (
                <div
                    ...
                    style={{
                        ...
                        transform:
                            mousePosition.x > chartRef.offsetWidth - 220
                                ...
                                {historyData
                                    ...
                                    .map((item, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                {item.buy
                                                    ? `${parseFloat(item.buy).toFixed(4)} PLN`
                                                    : '—'}
                                            </td>
                                            <td>
                                                {item.sell
                                                    ? `${parseFloat(item.sell).toFixed(4)} PLN`
                                                    : '—'}
                                            </td>
                                        </tr>
                                    ))}
